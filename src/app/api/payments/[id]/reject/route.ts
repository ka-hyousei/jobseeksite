...
export async function GET(
  ...
  try {
    ...
    console.log(`[Payment] Payment rejected: ${approval.paymentId} by ${process.env.ADMIN_EMAIL}`)
      ...
      generateHTML(
        ...
        `
          ...
          <div style="background: #f3f4f6; padding: 20px; border-radius: 8px; margin: 20px 0;">
            ...
            <p><strong>却下日時:</strong> ${new Date().toLocaleString('ja-JP')}</p>
          </div>
          <p>この支払いは無効となりました。</p>
        `,
        'error'
    ...
    return new NextResponse(
      generateHTML('エラー', '支払いの却下中にエラーが発生しました。', 'error'),
      {
        status: 500,
        headers: { 'Content-Type': 'text/html; charset=utf-8' }
      }
